...
| Gate D guest relay live under KVM/OVMF | Implemented (Phase 28: `build-guests` in live boot chain, Gate D serial marker smoke; validate-only default) |
| DMAR MMIO / guest datapath | Partial (Phases 18–27: synthetic IPC forward + e1000 MMIO smoke, mock/wall-clock benchmark, guest-runtime relay, freestanding source-tree guests with sustained relay loops, boot-info/RDI handoff, live VMX guest execution scaffolding, in-VM mock throughput benchmark + host sustained relay validation; live in-VM relay frame measurement deferred) |
