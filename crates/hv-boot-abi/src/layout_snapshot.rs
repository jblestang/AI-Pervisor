...
pub struct LayoutSnapshot {
    ...
    pub guest_regions: [PlannedRegionSnapshot; MAX_LAYOUT_GUEST_REGIONS],
    /// Number of valid entries in [`Self::ipc_regions`].
    pub ipc_region_count: u32,
    /// IPC shared memory regions sorted by host address.
    pub ipc_regions: [PlannedRegionSnapshot; MAX_LAYOUT_IPC_REGIONS],
...
mod tests {
    ...
    fn layout_snapshot_layout_is_stable() {
        ...
        assert_eq!(size_of::<LayoutPciSnapshot>(), 12);
        assert_eq!(align_of::<LayoutSnapshot>(), 8);
        assert!(size_of::<LayoutSnapshot>() > size_of::<PlannedRegionSnapshot>());
    }
}
