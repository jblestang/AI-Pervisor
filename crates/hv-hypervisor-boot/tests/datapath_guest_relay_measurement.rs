//! Gate D datapath guest relay measurement orchestration tests (requires `datapath-guest-relay-measurement`).

#![allow(clippy::expect_used, clippy::indexing_slicing)]

use hv_config_model::compile_config_from_str;
use hv_datapath::{
    DatapathRuntimeDisposition, GuestThroughputDisposition, GUEST_RELAY_BENCHMARK_FRAMES,
};
...
fn assert_datapath_guest_relay_live_validate_only(result: &GateDDatapathGuestRelayLiveResult) {
    assert_eq!(
    ...
    assert!(!result.throughput_seam.live_relay_validated);
...
#[test]
fn boot_from_transfer_and_init_gate_d_datapath_guest_relay_live_accepts_reference_transfer() {
    ...
    let mut allocator = MockPageAllocator::new(0x0000_0000_2700_0000);
    let result = boot_from_transfer_and_init_gate_d_datapath_guest_relay_live(
        &transfer,
        &snapshot,
        &layout,
        &mut allocator,
    )
    .expect("guest relay live");
    assert_datapath_guest_relay_live_validate_only(&result);
}

#[test]
fn boot_from_transfer_and_init_gate_d_datapath_guest_relay_live_from_snapshots_accepts_reference_transfer(
) {
    if !GUEST_SOURCE_ELFS_AVAILABLE {
        eprintln!("skipping: run cargo xtask build-guests to embed source ELFs");
        return;
    ...
    let mut allocator = MockPageAllocator::new(0x0000_0000_2800_0000);
    ...
    .expect("guest relay live snapshots");
    assert_datapath_guest_relay_live_validate_only(&result);
}
