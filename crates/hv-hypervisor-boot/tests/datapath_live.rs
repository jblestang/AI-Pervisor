//! Gate D datapath live orchestration tests (requires `datapath-live` feature).
...
use hv_hypervisor_boot::{
    ...
    layout_snapshot_from_platform_ir, requirements_snapshot_from_platform, GateDDatapathLiveResult,
};
use hv_loader::{
...
#[test]
fn boot_from_transfer_and_init_gate_d_datapath_live_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let mut allocator = MockPageAllocator::new(0x0000_0000_0E00_0000);
    let result = boot_from_transfer_and_init_gate_d_datapath_live(
        &transfer,
        &snapshot,
        &layout,
        &mut allocator,
    )
    .expect("datapath live");
    assert_datapath_live_validate_only(&result);
}

#[test]
fn boot_from_transfer_and_init_gate_d_datapath_live_from_snapshots_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let layout_snapshot = layout_snapshot_from_platform_ir(&layout).expect("layout snapshot");
    let mut allocator = MockPageAllocator::new(0x0000_0000_0F00_0000);
    ...
    .expect("datapath live snapshots");
    assert_datapath_live_validate_only(&result);
}
