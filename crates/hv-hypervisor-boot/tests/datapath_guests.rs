//! Gate D datapath guests orchestration tests (requires `datapath-guests` feature).
...
fn assert_datapath_guests_validate_only(result: &GateDDatapathGuestsResult) {
    assert!(result.malicious.integrity_checks_passed);
    assert_eq!(
...
#[test]
fn boot_from_transfer_and_init_gate_d_datapath_guests_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let mut allocator = MockPageAllocator::new(0x0000_0000_1400_0000);
    let result = boot_from_transfer_and_init_gate_d_datapath_guests(
        &transfer,
        &snapshot,
        &layout,
        &mut allocator,
    )
    .expect("datapath guests");
    assert_datapath_guests_validate_only(&result);
}

#[test]
fn boot_from_transfer_and_init_gate_d_datapath_guests_from_snapshots_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let layout_snapshot = layout_snapshot_from_platform_ir(&layout).expect("layout snapshot");
    let mut allocator = MockPageAllocator::new(0x0000_0000_1500_0000);
    ...
    .expect("datapath guests snapshots");
    assert_datapath_guests_validate_only(&result);
}
