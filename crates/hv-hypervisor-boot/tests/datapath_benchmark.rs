//! Gate D datapath benchmark orchestration tests (requires `datapath-benchmark` feature).

#![allow(clippy::expect_used, clippy::indexing_slicing)]

use hv_config_model::compile_config_from_str;
use hv_datapath::{BENCHMARK_MIN_RUNS, TARGET_THROUGHPUT_MBIT_PER_SEC};
...
fn assert_datapath_benchmark_validate_only(result: &GateDDatapathBenchmarkResult) {
    assert!(result.guests.malicious.integrity_checks_passed);
    assert_eq!(
        result.guests.elf_images_installed,
        REFERENCE_GUEST_PARTITION_IDS.len() as u32
    );
    assert_eq!(result.benchmark.runs_completed, BENCHMARK_MIN_RUNS);
...
#[test]
fn boot_from_transfer_and_init_gate_d_datapath_benchmark_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let mut allocator = MockPageAllocator::new(0x0000_0000_1700_0000);
    let result = boot_from_transfer_and_init_gate_d_datapath_benchmark(
        &transfer,
        &snapshot,
        &layout,
        &mut allocator,
    )
    .expect("datapath benchmark");
    assert_datapath_benchmark_validate_only(&result);
}
...
fn boot_from_transfer_and_init_gate_d_datapath_benchmark_from_snapshots_accepts_reference_transfer()
{
    ...
    .expect("datapath benchmark snapshots");
    assert_datapath_benchmark_validate_only(&result);
}
