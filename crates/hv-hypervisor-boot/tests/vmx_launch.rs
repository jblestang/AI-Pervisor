//! Gate C VMX launch orchestration tests (requires `vmx-launch` feature).
...
fn assert_vmx_launch_validate_only(result: &GateCVmxLaunchResult) {
...
#[test]
fn boot_from_transfer_and_init_gate_c_vmx_launch_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let mut allocator = MockPageAllocator::new(0x0000_0000_0900_0000);
    let result = boot_from_transfer_and_init_gate_c_vmx_launch(
        &transfer,
        &snapshot,
        &layout,
        &mut allocator,
    ...
    .expect("vmx launch");
    assert_vmx_launch_validate_only(&result);
}

#[test]
fn boot_from_transfer_and_init_gate_c_vmx_launch_from_snapshots_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let layout_snapshot = layout_snapshot_from_platform_ir(&layout).expect("layout snapshot");
    let mut allocator = MockPageAllocator::new(0x0000_0000_0A00_0000);
