//! Gate D datapath guest-live orchestration tests (requires `datapath-guest-live`).
...
#[test]
fn boot_from_transfer_and_init_gate_d_datapath_guest_live_accepts_reference_transfer() {
    ...
    let mut allocator = MockPageAllocator::new(0x0000_0000_1F00_0000);
    let result = boot_from_transfer_and_init_gate_d_datapath_guest_live(
        &transfer,
        &snapshot,
        &layout,
        &mut allocator,
...
#[test]
fn boot_from_transfer_and_init_gate_d_datapath_guest_live_from_snapshots_accepts_reference_transfer(
) {
    if !GUEST_SOURCE_ELFS_AVAILABLE {
        eprintln!("skipping: run cargo xtask build-guests to embed source ELFs");
        return;
    ...
    let mut allocator = MockPageAllocator::new(0x0000_0000_2000_0000);
    ...
    .expect("guest live snapshots");
    assert_eq!(result.boot_infos_installed, 3);
}
