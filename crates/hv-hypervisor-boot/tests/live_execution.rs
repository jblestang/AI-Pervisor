//! Gate C live execution orchestration tests (requires `live-execution` feature).
...
#[test]
fn boot_from_transfer_and_init_gate_c_live_execution_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let result = boot_from_transfer_and_init_gate_c_live_execution(&transfer, &snapshot, &layout)
        .expect("live execution");
    assert!(!result.live_environment_ready);
    assert!(result.cpu_seam.programming.vmxon_region.is_some());
    if let Some(seam) = result.cpu_seam.vmx_seam {
        assert_ne!(seam.disposition, CpuInstructionDisposition::Executed);
    }
}

#[test]
fn boot_from_transfer_and_init_gate_c_live_execution_from_snapshots_accepts_reference_transfer() {
    let (transfer, snapshot, layout) = reference_handoff_snapshot_and_layout();
    let layout_snapshot = layout_snapshot_from_platform_ir(&layout).expect("layout snapshot");
    let result = boot_from_transfer_and_init_gate_c_live_execution_from_snapshots(
        &transfer,
        &snapshot,
        &layout_snapshot,
    )
    .expect("live execution snapshots");
    assert!(result.cpu_seam.programming.init.validated.observed.vtd);
}

#[test]
fn boot_check_and_init_gate_c_live_execution_accepts_reference_inputs() {
    let yaml = include_str!("../../../configs/qemu.yaml");
    let compiled = compile_config_from_str(yaml).expect("compile");
    let firmware = encode_qemu_reference_firmware();
    let handoff = build_loader_handoff(
    ...
    .expect("handoff");
    let layout = plan_static_platform_ir(&compiled.intent).expect("plan");
    let result = boot_check_and_init_gate_c_live_execution(
