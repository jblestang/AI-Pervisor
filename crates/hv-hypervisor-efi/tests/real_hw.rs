//! REAL_HW hypervisor EFI entry tests.

#![allow(clippy::expect_used, clippy::indexing_slicing)]

use hv_config_model::compile_config_from_str;
use hv_hypervisor_efi::boot_hypervisor_from_transfer_real_hw;
...
#[test]
fn boot_hypervisor_from_transfer_real_hw_accepts_reference_handoff() {
    ...
    .expect("real hw boot");
    assert!(!markers.vmxon_executed);
    assert!(!markers.ept_executed);
}

#[test]
fn boot_hypervisor_from_transfer_real_hw_rejects_digest_mismatch() {
    let yaml = include_str!("../../../configs/qemu.yaml");
    let compiled = compile_config_from_str(yaml).expect("compile");
    let layout = plan_static_platform_ir(&compiled.intent).expect("plan");
    let mut requirements = requirements_snapshot_from_platform(
        &compiled.requirements,
        compiled.digest.bytes,
        layout.hypervisor_reserve.host_phys.raw(),
        layout.hypervisor_reserve.size.bytes(),
    ...
    assert!(boot_hypervisor_from_transfer_real_hw(
        &transfer,
        &compiled.digest.bytes,
        &requirements,
        &layout_snapshot,
