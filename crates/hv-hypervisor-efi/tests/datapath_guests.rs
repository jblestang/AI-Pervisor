//! Gate D datapath guests hypervisor EFI entry tests.

#![allow(clippy::expect_used, clippy::indexing_slicing)]

use hv_config_model::compile_config_from_str;
use hv_guest_boot::REFERENCE_GUEST_PARTITION_IDS;
use hv_hypervisor_efi::boot_hypervisor_from_transfer_datapath_guests;
...
#[test]
fn boot_hypervisor_from_transfer_datapath_guests_accepts_reference_handoff() {
    ...
    let mut allocator = MockPageAllocator::new(0x0000_0000_1600_0000);
    ...
    .expect("datapath guests boot");
    assert!(markers.malicious.integrity_checks_passed);
    assert_eq!(
        markers.elf_images_installed,
        REFERENCE_GUEST_PARTITION_IDS.len() as u32
    );
    assert!(markers.multi_partition_vmlaunch);
    assert!(!markers.malicious.live.foundation.vmx_launch.vmlaunch_executed);
}
