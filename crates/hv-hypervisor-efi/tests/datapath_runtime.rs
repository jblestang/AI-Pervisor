//! Gate D datapath runtime hypervisor EFI entry tests.
...
#[test]
fn boot_hypervisor_from_transfer_datapath_runtime_accepts_reference_handoff() {
    ...
    .expect("datapath runtime boot");
    assert!(markers.benchmark.guests.malicious.integrity_checks_passed);
    assert!(markers.benchmark.benchmark_target_met);
    assert_eq!(
        markers.datapath_elf_images_installed,
